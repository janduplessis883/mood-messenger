...
def load_model1():
    ...
    ui.badges(badge_list=[("Loaded Sentiment Analysis Model from Huggingface", "default")], class_name="flex gap-2", key="badges1")
    return sentiment_task

@st.cache_resource(experimental_allow_widgets=True)
def load_model2():
    classifier = pipeline(task="text-classification", model="SamLowe/roberta-base-go_emotions", top_k=2)
    ui.badges(badge_list=[("Loaded Emotions Model from Huggingface", "default")], class_name="flex gap-2", key="badges2")
    return classifier
...
def sentiment_analysis(data, column, sentiment_task):
    ...
    for index, row in tqdm(data.iterrows(), total=total_rows, desc="Analyzing Sentiment"):
        sentence = row[column]
        sentence = str(sentence)[:513]  # Ensure the text is a string and truncate if necessary
...
if uploaded_file is not None and st.session_state.final_dataframe is None:
    ...
    # Parse the chat data
    df[['datetime', 'from', 'chat']] = df['chat'].apply(lambda x: pd.Series(parse_chat(x)))
